...
def generate_fixed_variable_voltage_question():
    ...
    if target == "fixed":
        ...
        resistor_name = "fixed resistor"
    else:
        target_resistance = variable_resistance
        resistor_name = "variable resistor"
...
def generate_fixed_variable_missing_voltage_question():
    ...
    for supply_voltage in possible_supply_voltages:
        ...
        fixed_voltage = round(supply_voltage * fixed_resistance / total_resistance, 2)
        variable_voltage = round(supply_voltage * variable_resistance / total_resistance, 2)

        if fixed_voltage * 2 == int(fixed_voltage * 2) and variable_voltage * 2 == int(variable_voltage * 2):
            valid_cases.append((supply_voltage, fixed_voltage, variable_voltage))

    if not valid_cases:
        return generate_fixed_variable_missing_voltage_question()
...
Voltage across {given_name} = {given_voltage} V

Find voltage across {target_name}.
"""
